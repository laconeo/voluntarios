...
import { Plus, Trash2, Calendar, Clock, Users, Tag, AlertCircle } from 'lucide-react';
...
const ShiftManagement: React.FC<ShiftManagementProps> = ({ eventId, eventStartDate, eventEndDate }) => {
    ...
    const handleAddShift = async () => {
        if (!newShift.date || !newShift.roleId) {
            toast.error('Por favor completa todos los campos');
            return;
        }

        // Validar que la fecha esté dentro del rango del evento
        if (newShift.date < eventStartDate || newShift.date > eventEndDate) {
        ...
        try {
            const shiftData: Omit<Shift, 'id'> = {
                ...
                roleId: newShift.roleId,
                totalVacancies: newShift.totalVacancies,
            ...
            {showAddModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
                        <h3 className="text-xl font-serif font-bold text-gray-900 mb-4">Agregar Nuevo Turno</h3>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <select
                                    value={newShift.roleId}
                                    onChange={(e) => setNewShift({ ...newShift, roleId: e.target.value })}
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                                >
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    min="1"
                                    max="100"
                                    value={newShift.totalVacancies}
                                    onChange={(e) => setNewShift({ ...newShift, totalVacancies: parseInt(e.target.value) || 1 })}
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                                />
                                <p className="text-xs text-gray-500 mt-1">
                        ...
                        <div className="flex gap-3 mt-6">
                            ...
                            <button
                                ...
                                disabled={!newShift.date || !newShift.roleId || roles.length === 0}
